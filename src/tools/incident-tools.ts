...
const SearchIncidentSchema = z.object({
  ...
  priority: z
    .string()
    .optional()
    .describe("Filter by incident priority (e.g., 'CRITICAL', 'HIGH', 'MEDIUM', 'LOW')"),
  entityGuid: z
    .string()
    .optional()
    .describe("Filter by entity GUID"),
  sinceDays: z
    .number()
    .int()
    .min(1)
...
const AnalyzeDeploymentImpactSchema = z.object({
  ...
  deploymentTimestamp: z
    .number()
    .optional()
    .describe("Unix timestamp (ms) of the deployment to analyze. If not provided, uses the most recent deployment."),
  beforeMinutes: z
    .number()
    .int()
    .min(5)
...
const GetEntityErrorGroupsSchema = z.object({
  entityGuid: z
    .string()
    .optional()
    .describe("Filter errors by entity GUID"),
  transactionName: z
    .string()
    .optional()
  ...
  limit: z
    ...
    .optional()
    .default(50)
    .describe("Maximum number of error groups to return (default: 50, max: 100)"),
});
...
const ListChangeEventsSchema = z.object({
  entityGuid: z
    .string()
    .optional()
    .describe("Filter changes by entity GUID"),
  changeType: z
    .string()
    .optional()
  ...
  limit: z
    ...
    .optional()
    .default(50)
    .describe("Maximum number of change events to return (default: 50, max: 200)"),
});
 ...
 */
export function registerGenerateAlertInsightsReportTool(server: McpServer): void {
  server.tool(
...
function formatDeploymentImpactResult(result: {
  ...
  lines.push(`- **Timestamp:** ${new Date(result.deployment.timestamp).toISOString()}`);
  ...
  if (result.impact.hasNegativeImpact) {
    ...
    if (result.impact.errorRateChange > 10) {
      lines.push(`- Error rate increased by ${result.impact.errorRateChange.toFixed(1)}%`);
    }
    if (result.impact.responseTimeChange > 20) {
      lines.push(`- Response time increased by ${result.impact.responseTimeChange.toFixed(1)}%`);
    }
    lines.push("");
    ...
    if (result.impact.errorRateChange < -5) {
      lines.push(`- Error rate improved by ${Math.abs(result.impact.errorRateChange).toFixed(1)}%`);
    }
    if (result.impact.responseTimeChange < -10) {
      lines.push(`- Response time improved by ${Math.abs(result.impact.responseTimeChange).toFixed(1)}%`);
    }
  }
...
function formatAlertInsightsReport(result: {
  ...
  if (result.errorGroups.length > 0) {
    ...
    for (const eg of result.errorGroups.slice(0, 10)) {
      ...
      const message = eg.errorMessage.length > 50 ? eg.errorMessage.substring(0, 47) + "..." : eg.errorMessage;
      lines.push(`| ${eg.errorClass} | ${message} | ${eg.count} | ${lastSeen} |`);
    }
    lines.push("");
  }
  ...
  // Active Issues (top 5)
  const activeIssues = result.issues.filter((i) => i.state === "ACTIVATED" || i.state === "CREATED");
  if (activeIssues.length > 0) {
    ...
    for (const issue of activeIssues.slice(0, 5)) {
      ...
      lines.push(`- **Activated:** ${new Date(issue.activatedAt).toISOString()}`);
...
function formatErrorGroupsResult(errorGroups: ErrorGroup[]): string {
  ...
  for (const eg of errorGroups) {
    const firstSeen = eg.firstSeen ? new Date(eg.firstSeen).toISOString() : "N/A";
    const lastSeen = eg.lastSeen ? new Date(eg.lastSeen).toISOString() : "N/A";
    const message =
      eg.errorMessage.length > 60 ? eg.errorMessage.substring(0, 57) + "..." : eg.errorMessage;
    lines.push(`| ${eg.errorClass} | ${message} | ${eg.count} | ${firstSeen} | ${lastSeen} |`);
  }

...
import { z } from "zod";
import { type LogQueryResult, getLogService } from "../services/log-service.js";
import {
	type NrqlQueryResult,
	getNrqlService,
} from "../services/nrql-service.js";
...
export const AnalyzeEntityLogsSchema = {
	entityGuid: z.string().min(1).describe("The entity GUID to filter logs by"),
	limit: z
		.number()
		.min(1)
		.max(2000)
...
export const NaturalLanguageToNrqlSchema = {
	...
	includeExamples: z
		...
		.optional()
		.describe(
			"Include example queries for the identified pattern (default: true)",
		),
};
...
function analyzeNaturalLanguage(
	...
	patterns: (typeof NRQL_PATTERNS)[keyof typeof NRQL_PATTERNS][];
	suggestions: string[];
} {
	...
	if (lowerDesc.match(/\b(over time|trend|timeseries|graph|chart)\b/)) {
		suggestions.push(
			"Time series detected: Add TIMESERIES clause for graphing",
		);
	}
	...
	if (matchedPatterns.length === 0) {
		suggestions.push(
			"No specific pattern detected. Starting with a general query template.",
		);
	}
		...
		async (args) => {
			try {
				...
				const guidance = {
					...
					nrqlSyntaxReference: {
						basicStructure:
							"SELECT <fields> FROM <event_type> WHERE <conditions> FACET <grouping> SINCE <time_range>",
						commonEventTypes: [

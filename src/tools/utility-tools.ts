...
const ConvertTimePeriodSchema = z.object({
	timeString: z
		.string()
		.describe(
			"Human-readable time string to convert. Examples: '1 hour ago', '30 minutes ago', '7 days ago', '2024-01-15', '2024-01-15T10:30:00Z'",
		),
	timezone: z
		.string()
		.optional()
		.default("UTC")
		.describe(
			"Timezone for interpreting relative times. Default: UTC. Examples: 'America/Los_Angeles', 'America/New_York', 'Europe/London'",
		),
	referenceTime: z
		.number()
		.optional()
		.describe(
			"Reference timestamp in milliseconds for relative calculations. Defaults to current time.",
		),
});
	...
	server.tool(
		...
		async (params) => {
			try {
				...
				defaultLogger.error(
					"convert_time_period_to_epoch_ms tool failed",
					error,
				);
				throw error;
			}

...
const ListAlertPoliciesSchema = z.object({
  nameFilter: z
    .string()
    .optional()
    .describe("Optional filter to search policies by name (case-insensitive partial match)"),
  limit: z
    .number()
    .int()
    .min(1)
...
const ListAlertConditionsSchema = z.object({
  policyId: z
    .string()
    .optional()
    .describe("Filter conditions by policy ID"),
  nameFilter: z
    .string()
    .optional()
    .describe("Optional filter to search conditions by name (case-insensitive partial match)"),
  enabledOnly: z
    .boolean()
    .optional()
    .default(false)
  ...
  limit: z
    ...
    .optional()
    .default(100)
    .describe("Maximum number of conditions to return (default: 100, max: 500)"),
});
...
function formatAlertPoliciesResult(result: {
  ...
  lines.push(`# Alert Policies (${result.policies.length} of ${result.totalCount} total)`);
  lines.push("");
  ...
  for (const policy of result.policies) {
    ...
    lines.push(`- **Incident Preference:** ${formatIncidentPreference(policy.incidentPreference)}`);
    lines.push("");
  }
...
function formatAlertConditionsResult(result: {
  ...
  lines.push(`# Alert Conditions (${result.conditions.length} of ${result.totalCount} total)`);
  lines.push("");
  ...
  for (const condition of result.conditions) {
    ...
    if (condition.signal) {
      if (condition.signal.aggregationWindow) {
        lines.push(`- **Aggregation Window:** ${condition.signal.aggregationWindow}s`);
      }
      if (condition.signal.evaluationOffset) {
        lines.push(`- **Evaluation Offset:** ${condition.signal.evaluationOffset}s`);
      }
    }
...
function formatIssue(issue: {
  ...
  lines.push(`- **Activated At:** ${new Date(issue.activatedAt).toISOString()}`);

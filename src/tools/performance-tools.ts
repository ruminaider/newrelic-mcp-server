...
const AnalyzeGoldenMetricsSchema = z.object({
	guid: z.string().describe("The entity GUID to analyze golden metrics for"),
});
...
const AnalyzeTransactionsSchema = z.object({
	since: z
		...
		.default("1 hour ago")
		.describe(
			"Time range start (NRQL SINCE clause). Examples: '1 hour ago', '24 hours ago', '7 days ago', '2024-01-01'",
		),
	until: z
		.string()
		.optional()
		.describe(
			"Time range end (NRQL UNTIL clause). Examples: 'now', '1 hour ago', '2024-01-02'",
		),
	facets: z
		.array(z.string())
		.optional()
		.default(["name"])
		.describe(
			"Fields to group results by. Default: ['name']. Common: 'name', 'host', 'request.uri'",
		),
	...
	where: z
		...
		.optional()
		.describe(
			'Additional WHERE clause conditions. Example: "duration > 1" or "error IS true"',
		),
	metrics: z
		.array(
			z.enum([
		...
		.describe(
			"Metrics to calculate. Options: count, averageDuration, totalTime, errorRate, throughput",
		),
	limit: z
		.number()
		.int()
			...
			try {
				...
				for (const result of response.results) {
					...
					if (result.averageDuration !== undefined) {
						...
						summary.maxDuration = Math.max(
							summary.maxDuration,
							result.averageDuration,
						);
						summary.minDuration = Math.min(
							summary.minDuration,
							result.averageDuration,
						);
						durationCount++;
				...
				const formattedResponse = {
					...
					summary: {
						...
						averageDurationMs:
							Math.round(summary.avgDuration * 1000 * 100) / 100,

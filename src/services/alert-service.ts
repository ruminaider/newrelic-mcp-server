...
export async function listAlertPolicies(options: {
	...
	const accountId = Number.parseInt(client.getAccountId(), 10);
	const policies: AlertPolicy[] = [];
	let cursor: string | null = null;
	const limit = options.limit ?? 100;

	defaultLogger.debug("Listing alert policies", {
	...
	do {
		...
		const response: PolicyResponse = await client.query<PolicyResponse>(
		...
		const result: PolicyResponse["actor"]["account"]["alerts"]["policiesSearch"] =
			response.actor.account.alerts.policiesSearch;
...
export async function listAlertConditions(options: {
	...
	do {
		...
		const response: ConditionResponse = await client.query<ConditionResponse>(
			LIST_ALERT_CONDITIONS_QUERY,
			{
				accountId,
				searchCriteria:
					Object.keys(searchCriteria).length > 0 ? searchCriteria : undefined,
		...
		const result: ConditionResponse["actor"]["account"]["alerts"]["nrqlConditionsSearch"] =
			response.actor.account.alerts.nrqlConditionsSearch;
		...
		if (options.enabledOnly) {
			filteredConditions = filteredConditions.filter(
				(c: NrqlAlertCondition) => c.enabled,
			);
		}
  ...
  `;
	...
	const errorRateChange =
		metrics.errorRateBefore > 0
			? ((metrics.errorRateAfter - metrics.errorRateBefore) /
					metrics.errorRateBefore) *
				100
	...
	const throughputChange =
		metrics.throughputBefore > 0
			? ((metrics.throughputAfter - metrics.throughputBefore) /
					metrics.throughputBefore) *
				100
			: 0;

	const responseTimeChange =
		metrics.responseTimeBefore > 0
			? ((metrics.responseTimeAfter - metrics.responseTimeBefore) /
					metrics.responseTimeBefore) *
				100
			: 0;

	// Negative impact if error rate increased significantly or response time increased
	const hasNegativeImpact = errorRateChange > 10 || responseTimeChange > 20;
...
export async function generateAlertInsightsReport(options: {
	...
	const [issuesResult, errorGroupsResult, deploymentsResult] =
		await Promise.all([
			listRecentIssues({
	...
	const summary = {
		...
		resolvedIssues: issues.filter(
			(i) => i.state === "CLOSED" || i.state === "DEACTIVATED",
		).length,
	};
